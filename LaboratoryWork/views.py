...
class ArticleList(View):
    def get(self, request, *args, **kwargs):
        ...
        context = { 'articles': articles,
                    ...
                    'is_moderator': user.is_teacher() }
        return render(request, 'Article/articles_table_page.html', context)

class ArticleView(View):
    def get(self, request, pk):
        this_article = Article.objects.get(id=pk)
        is_moder = request.user.groups.filter(name='teacher').exists()
...
class LabWork(View):
    def get(self, request):
        lab_works = LaboratoryWork.objects.all()
        is_teacher = request.user.groups.filter(name='teacher').exists()

        context = { 'lab_works': lab_works,
                    'is_teacher': is_teacher }
        return render(request, 'Article/lab_works_page.html', context)
